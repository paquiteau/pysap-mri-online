...
def get_operators(
    ...
    if fourier_type == 0:  # offline reconstruction
        kspace_generator = KspaceGeneratorBase(
            full_kspace=kspace_data, mask=mask, max_iter=max_iter
        )
        fourier_op = FFT(shape=shape, n_coils=n_coils, mask=mask)
    elif fourier_type == 1:  # online type I reconstruction
        kspace_generator = Column2DKspaceGenerator(
            full_kspace=kspace_data, mask_cols=loc
        )
        fourier_op = FFT(shape=shape, n_coils=n_coils, mask=mask)
    elif fourier_type == 2:  # online type II reconstruction
        kspace_generator = DataOnlyKspaceGenerator(
            full_kspace=kspace_data, mask_cols=loc
        )
    ...
    if regularisation is not None:
        ...
        if reg_cls == "GroupLASSO":
            ...
            prox_op = OWL(
                **regularisation, n_coils=n_coils, bands_shape=linear_op.coeffs_shape
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Grid search of the MRI online program"
    )
    parser.add_argument(
        "-f",
        dest="FORCE",
        action="store_true",
        default=False,
        help="recomputed past computations",
    )
    parser.add_argument(
        "grid_file",
        type=str,
        default="result/grid_config.yml",
        help="config file setting data, problem and solver configuration.",
    )
    parser.add_argument(
        "--dry",
        dest="DRY",
        action="store_true",
        default=False,
        help="dry mode, skip the computation, but do everything else",
    )
    parser.add_argument(
        "--tested",
        type=str,
        default="results/tested_config.yml",
        help="List of past config.",
    )
    parser.add_argument(
        "--failed",
        type=str,
        default="results/failed_config.yml",
        help="List of past, failed config.",
    )
    ...
    for idx, setup in enumerate(setups):
        ...
        if not args.DRY:
            ...
            ksp, fft, lin, prox = get_operators(
                kspace_data=final_k, loc=mask_loc, mask=final_mask, **problem
            )
            alg_name = solver.pop("algo")
            online_pb = OnlineReconstructor(
                fft, lin, regularizer_op=prox, opt=alg_name, verbose=0
            )
            # configure metrics
            metrics_config = create_cartesian_metrics(
                online_pb, real_img, final_mask, final_k, estimates=5
            )
