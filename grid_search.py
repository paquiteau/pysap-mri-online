...
fourier = [(kspace_gen, fourier_op, 1), (kspace_gen2, fourier_op2, 2)]
...
for idx, (cs, c) in enumerate(configs):
    ...
    if not e.has_saved_results or FORCE_COMPUTE:
        print(c['opt'])
        online_pb = OnlineReconstructor(fourier_op=c['fourier_op'],
        ...
        xf, costs, metrics_results = online_pb.reconstruct(c['kspace_gen'], cost_op_kwargs={'cost_interval': 1},
                                                           metrics=metrics, metric_call_period=1, **c['alg_kwargs'])
