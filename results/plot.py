...
def get_label(e, disc_keys,pres=2):
    """
    ...
    return key_val(val_pres=pres, **legend_key)
...
def get_color_list(e, disc, colors, hash_table, pres=2):
    label = hash(get_label(e, disc,pres=pres))
...
def plot_metrics(
    ...
    if default_offset is None:
        default_offset = (80, 0, 1)
    elif isinstance(default_offset, int):
        default_offset = (default_offset, default_offset, default_offset)
    if max_iter is None:
        max_iter = (-1, -1, -1)
    ...
    for idx, exp in enumerate(dataset):
        ...
        iters = max_iter[exp.problem["fourier_type"]]
        cl = get_color_list(exp, disc_clean_colors, colors, color_hash_table,pres=val_pres)
        ...
        lbl = get_label(exp, disc_clean, pres=val_pres)
        for ax, metric in zip(axs, metrics):
            data = getattr(exp.results, metric)
            if log:
                ax.semilogy(
                    np.arange(len(data[:iters])) + offset,
                    data[:iters],
                    linestyle=ls,
                    color=cl,
                    linewidth=lw,
            ...
            ax.set_ylabel(METRIC_PRETTY.get(metric,metric))
            if default_offset:
                ax.axvline(
                    x=default_offset[0], linestyle="solid", color="gray", linewidth=0.2
                )
    # legend clean_up
    if len(dataset) > 1:
        ...
        labels, handles = zip(
            *sorted(zip(unique_labels, unique_handles), key=lambda t: t[0])
        )
...
#             bbox_transform=fig.transFigure,
    ...
    if title:
        fig.suptitle(key_val(val_pres=val_pres,**dataset.get_discriminant_param(disc=False)))
    else:
        print(key_val(val_pres=val_pres,**dataset.get_discriminant_param(disc=False)))
    fig.tight_layout()
    return fig
