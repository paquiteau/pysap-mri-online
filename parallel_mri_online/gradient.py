...
class Grad2D_pMRI_analysis(GradBasic, PowerMethod):
    ...
    def __init__(self, data, fourier_op):
        ...
        GradBasic.__init__(
            self, data, self._analy_op_method, self._analy_rsns_op_method
        )
    ...
    def _analy_op_method(self, x):
        ...
        """
        data = []
        [data.append(self.fourier_op.op(x[channel])) for channel in range(x.shape[0])]
...
class Grad2D_pMRI_synthesis(GradBasic, PowerMethod):
    ...
    def __init__(self, data, fourier_op, linear_op):
        ...
        GradBasic.__init__(
            self, data, self._synth_op_method, self._synth_trans_op_method
        )
        coef = linear_op.op(
            np.zeros((data.shape[0], *fourier_op.shape)).astype(np.complex)
        )
    ...
    def _synth_trans_op_method(self, x):
        ...
        """
        images = np.asarray(
            [self.fourier_op.adj_op(x[channel]) for channel in range(x.shape[0])]
        )
...
class Grad2D_pMRI(Grad2D_pMRI_analysis, Grad2D_pMRI_synthesis):
    ...
    * (1/2) * (||Ft L* alpha - yl||^2_2,l)
    ...
    def get_cost(self, x):
        ...
        """
        return 0.5 * (np.abs(self.op(x).flatten() - self.obs_data.flatten()) ** 2).sum()
